...
public:
	SunBullet()
	{
		...
		animation_exploed.set_atlas(&atlas_sun_explode);
		animation_exploed.set_interval(50);
		animation_exploed.set_loop(false);
		animation_exploed.set_callback([&]() { can_remove = true; });

		IMAGE* frame_idle = animation_idle.get_frame();
		IMAGE* frame_explode = animation_exploed.get_frame();
	...
	void on_update(int delta)
	{
		...
		if (!valid)
			animation_exploed.on_update(delta);
		else
	...
	void on_draw(const Camera& camera) const
	{
		...
		{
			animation_exploed.on_draw(camera,
...
private:
	...
	Animation animation_exploed;   // 日光炸弹爆炸动画
